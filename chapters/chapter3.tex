...
\begin{figure}
    ...
    \caption[Application of the toy model with biologically plausible values to estimate the effect of softness on a real system.]{Estimation of the relative enhancement of the long-time diffusion coefficient of a Brownian colloid of radius $a=100\mathrm{nm}$ in a periodic potential of amplitude $\Delta U = 4 k_\mathrm{B}T$ and spatial period $\lambda = 100 \mathrm{nm}$ at a distance $d = 10 \mathrm{nm}$ from membrane of elastic modulus $E = 1 \mathrm{kPa}$ or equivalent dimensionless compliance $\epsilon = 0.013$. The hydrodynamic constant $\mu_{12}$ is estimated using Oseen's model. The two asymptotes of the very rigid $\epsilon \to 0$ and very soft $\epsilon \to \infty$ are shown in dash-dotted cyan and blue lines respectively. For this system, the expected value of the relative enhancement of the long-time diffusion coefficient of the Brownian colloid is of about $\frac{\Delta D^*}{D^*} \approx 100\%$.}
    \label{biovalues}
\end{figure}
