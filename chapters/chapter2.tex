...
with $\alpha = \cosh^{-1} \left[ (z+a)/a \right]$. Because Brenner obtained the effective viscosity from solving the full Stokes equations with a no-slip boundary condition at the wall, it is worth noting that this expression is exact and valid for all values of $z/a > 0$, that is to say up until colloid/plane contact. However, for the sake of efficient numerical computations later on, we avoid infinite-series computations and prefer the use of Padé approximants ~\cite{BakerGravesMorris_1996}, which were used by Honig ~\cite{Honig_1971}, Bevan and Prieve ~\cite{Bevan_2000} to approximate Brenner's result as:
...
At thermodynamic equilibrium, the local ion concentration in the diffuse layer $c_i(\vect{r})$ of a given species should follow the Boltzmann distribution:

\begin{equation}
c_i(\vect{r}) = c^\infty_{i} \exp\left\{-\frac{z_i e \psi(\vect{r})}{k_\mathrm{B} \Theta}\right\}~, 
...
\begin{equation}
\nabla^2 \psi(\vect{r}) = - \frac{1}{\varepsilon} \sum_i z_i e c_i(\vect{r})~,
\label{poisson_equation}
\end{equation}

where $\varepsilon$ is the permittivity of the fluid. Substituting the Boltzmann distribution for $c_i(\vect{r})$ into the Poisson equation yields the Poisson-Boltzmann equation, which is a non-linear partial differential equation for $\psi(\vect{r})$.

Thus, combining the above expressions, we get:

\begin{equation}
    \nabla^2 \psi(\vect{r}) + \frac{e}{\varepsilon} \sum_i z_i c^\infty_i \exp \left\{\frac{-z_i e \psi(\vect{r})}{k_\mathrm{B} \Theta} \right\} = 0~.
...
\begin{equation}
    \nabla^2 \psi(\vect{r}) + \frac{e}{\varepsilon} \sum_i z_i c^\infty_i  \left(1 - \frac{z_i e \psi(\vect{r})}{k_\mathrm{B} \Theta} \right) = 0~.
...
\begin{align}
    \nabla^2 \psi(\vect{r}) &= \left[\sum_i \frac{z_i^2e^2c^\infty_i}{\varepsilon k_\mathrm{B} \Theta}\right] \psi(\vect{r})\\
    & = \frac{1}{l_\mathrm{D}} \psi(\vect{r})~,
    \label{lD_psi}
\end{align}
...
This distribution should be normalised, hence the $1/Z$ factor. It is particularly useful to keep the shape of it in mind when designing a simulation, as depending on the effect one wants to probe (very dense suspension, monolayer, or inversely, virtually unbounded colloids in the $z$-plane), we may play with the parameters to get the desired behaviour.
...
\end{equation}
with $u(z) = (F)(\gamma_\perp)$ and, because $\lim_{z\to \infty} P_\mathrm{steady~state}(z) = 0$ and $\lim_{z\to \infty} (\mathrm{d} P_\mathrm{steady~state}(z)/\mathrm{d}z) = 0$,
\begin{equation}
...
\end{equation}
We express $\ln \left(g^2(z)\right) = \int \frac{2g'(z')g(z')}{g^2(z')}\mathrm{d}z'$, and:
\begin{equation}
    U_{steady-state}(z) = -k_\mathrm{B}\Theta \int\frac{u(z') + (\alpha - 1)g(z')g'(z')}{g^2(z')/2} \mathrm{d}z'~.
\end{equation}
With $F_\mathrm{steady~state} = -\mathrm{d}U_\mathrm{steady~state}/\mathrm{d}z$, we get:
\begin{equation}
\frac{F_\mathrm{steady~state}}{\gamma_\perp(z)} = \frac{F(z)}{\gamma_\perp(z)} + (\alpha - 1)g(z)g'(z)~.
\end{equation}

From the Lanvegin equation from Eq. \ref{confinedLangevin_stochdrift}, we note that $g(z) = \sqrt{2D_\perp(z)}$, and $F(z) = u(z)/\gamma_\perp$, and we also keep in mind that $D_\perp = k_\mathrm{B}\Theta/\gamma_\perp$. With the Itô convention $\alpha = 0$, we may now write the total drift of Eq. \ref{final_stochastic_drift}:
...
\begin{equation}
v_\mathrm{stoch} = 2D_0a\frac{2a^2 + 12 az + 21 z^2}{(2a^2 + 9az + z^2)^2}~.
...
\begin{align}
    - \nabla p(\vect{r}) + \eta \nabla^2\vect{u}(\vect{r}) &= -\vect{F} \delta(\vect{r})~,\\
    \nabla \cdot \vect{u} &= 0 % incompressibility
    \label{stokes_equations}
\end{align}
where $\nabla p$ is the pressure gradient in the fluid, that is to say, the force density due to pressure differences in the fluid; $\mu \nabla^2 \vect{u}$ is the viscous term that controls how the fluid diffuses momentum through itself with $\mu$ the fluid viscosity and $\vect{u}$ the fluid velocity field; $\vect{F} \delta(\vect{r})$ is the point force at origin with $\delta$ the Kronecker function; and the second equation denotes fluid incompressibility.

The velocity vector can be written as a product of the mobility tensor $\mat{M}$ and the force vector $\vect{f}$:

\begin{equation}
    \vect{v} = \mat{M} \cdot \vect{f}~,
    \label{mobility relation}
\end{equation}


where the mobility matrix $\mat{M}$ is of dimension $6N \times 6N$ and has the structure:

\begin{equation}
    \mat{M} = \begin{bmatrix} 
    \mu^{tt}_{11} & \dots & \mu^{tt}_{1N} & \mu^{tr}_{11}& \dots & \mu^{tr}_{1N}\\
    \vdots & \ddots &  \vdots & \vdots & \ddots &  \vdots\\
    \mu^{tt}_{N1} &   \dots     & \mu^{tt}_{NN} & \mu^{tr}_{N1} & \dots & \mu^{tr}_{NN}\\ 
    \mu^{rt}_{11} &   \dots     & \mu^{rt}_{1N} & \mu^{rr}_{11} & \dots & \mu^{rr}_{1N}\\ 
    \vdots & \ddots &  \vdots & \vdots & \ddots &  \vdots\\
    \mu^{rt}_{N1} &   \dots     & \mu^{rt}_{NN} & \mu^{rr}_{N1} & \dots & \mu^{rr}_{NN}~.
    \end{bmatrix}
\end{equation}

and $\mat{f} = \vect{F}_1, \dots, \vect{F}_N, \vect{T}_1, \dots, \vect{T}_N$ with $\vect{F}$ the forces, and $\vect{T}$ the torques. Subsequently,
\begin{align}
    \vect{v}_i &= \sum_{j}\left(\vect{\mu}^{tt}_{ij} \cdot \vect{F}_j + \vect{\mu}^{tr}_{ij} \cdot \vect{T}_j \right)~, \\
    \vect{w}_i &= \sum_{j}\left(\vect{\mu}^{rt}_{ij} \cdot \vect{F}_j + \vect{\mu}^{rr}_{ij} \cdot \vect{T}_j \right)~,
\end{align}

with $\vect{v}_i$ the translational and $\vect{w}_i$ rotational velocities of the colloid indexed $i$. It is worth mentioning that in our problems, the mobility matrix must be symmetric positive definite ~\cite{happel_low_1973}, which greatly simplifies its structure. 




\subsubsection{The Oseen tensor}
We consider the case of a colloid moving in an infinite and quiescent fluid. We aim to express the velocity field at any point in the fluid due to the displacement of this colloid. By effectively zooming out, we may approximate this colloid as a point force $\vect{F}_1$ acting upon the fluid at a given position $\vect{r_1}$. 

In that case, the velocity field $\vect{u(\vect{r})}$ is written:

\begin{equation}
    \vect{u}(\vect{r}) = \tens{G}(\vect{r} - \vect{r}_1) \cdot \vect{F}~,
    \label{Kim_U}
\end{equation}
where $\tens{G}$ is the Oseen tensor. Because $\vect{u}(\vect{r})$, is the fundamental solution of the Stokes equation (for a point force, that is), it is referred to as a "Stokeslet" ~\cite{kim_microhydrodynamics_2005}, and the Oseen tensor $\tens{G}[\vect(r) - \vect{r}_1]$ is the Green's function to the Stokes equations. We do not take into account rotation of the colloids, that we consider as perfect spheres. We however keep track of their orientation anyway, for the sake of the derivation and later on to check numerical accuracy. With this, we may now express the flow field generated for a continuous force in space as the superposition of Green's functions:

\begin{equation}
    \vect{u}(\vect{r}) = \int \tens{G}(\vect{r} - \vect{r'}) \cdot \vect{f}(\vect{r'}) \mathrm{d}^3 r'~.
    \label{Kim_U_continuous}
\end{equation}


Solving the Stokes equation, and taking into account symmetries ($1/(4\pi)$ factor) and incompressibility ($1/2$ factor), we then obtain:

\begin{equation}
    \tens{G}(\vect{r}) = \frac{1}{8 \pi \eta r} (\mat{I} + \vect{\hat{r}\hat{r}})~.
\end{equation}


Now, if we solve for two colloids at positions $\vect{x}$ and $\vect{r_j}$ in interaction in the fluid, we simply obtain:

\begin{equation}
    \tens{G} (\vect{x} - \vect{r}_j) = \frac{1}{8 \pi \eta} \left( \frac{1}{|\vect{x} - \vect{r_j}|} + \frac{(\vect{x} - \vect{r_{j}})(\vect{x} - \vect{r_{j}})}{|\vect{x} - \vect{r}_j|^3} \right)
...
where the $\alpha$ and $\beta$ components correspond respectively a sphere moving along the spheres lines of centers ($\parallel$) or perpendicularly to their lines of center ($\perp$) (Fig. \ref{perp_para_coll_rel}) ~\cite{Dufresne_2000}. Taking Eq. \ref{Kim_U}, we may identify the $\tens{G}$ tensor as a mobility tensor. In that case, a diffusion tensor between two colloids can be expressed with $\tens{G}$ as:

\begin{equation}
D_{i\alpha, j\beta} = k_\mathrm{B}\Theta G_{\alpha \beta} (\vect{r}_1 - \vect{r}_2)~.
\end{equation}
...
\begin{equation}
    \tens{\mu}^{\mathrm{RPY}}_{12}(\vect{r}) = \frac{1}{8 \pi \eta \vect{r}} \left[ \left(1 + \frac{2a^2}{3r^2}\right) \mat{I} + \left(1 - \frac{2a^2}{r^2} \frac{\vect{r}\vect{r}}{r^2} \right) \right]~.
\end{equation}
...
The following setup, depicted in Figure \ref{dufresne_system}, is used: two colloids of radius $a=0.485\pm0.025\, \upmu \mathrm{m}$ are submerged in a water-salt solution of 2mM [NaCl] and viscosity $\eta = 0.817\, \mathrm{cP}$. 
...
\begin{align}
    ...
    G^D_{\alpha\beta} (\vect{x} - \vect{R_j}) &= \frac{1}{8 \pi \eta} \left( 1 - 2 \delta_{\beta z} \right) \frac{\partial}{\partial R_{j, \beta}} \left( \frac{(\vect{x} - \vect{R_j})_\alpha}{|\vect{x} - \vect{R_j}|^3} \right)~,\\
    G^{SD}_{\alpha\beta} (\vect{x} - \vect{R_j}) &= (1 - 2\delta_{\beta z}) \frac{\partial}{\partial R_{j, \beta}} G^S_{\alpha z} (\vect{x} - \vect{R_j})~.
\end{align}
...
with $\xi = 4h^2 / r^2$ and up to $\mathcal{O} \left( a^3/r^3\right)$ and $\mathcal{O} \left( a^3/h^3\right)$ (in solid lines in Figure \ref{Dufresne_cross_diffusion}).
